...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select(
  { label, error, tone = 'light', options, placeholder, id, className, defaultValue, value, ...rest },
  ref,
) {
  const generatedId = useId();
  const selectId = id ?? generatedId;
  // Only fall back to an uncontrolled defaultValue when the caller isn't
  // passing `value` — setting both on the same <select> triggers a React
